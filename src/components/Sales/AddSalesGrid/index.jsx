...
import {
  getProductsRequest,
  setClickCounts,
  setDisplaySaleCategory,
  setSelectedSaleCategory,
} from "../../../config/store/actions/productActions";
import {
  addNewSalesRequest,
  setShowPayModal,
      ...
      case "foods":
        dispatch(
          setDisplaySaleCategory(
            allProducts.filter(
              (item) =>
                item.food_category === selectedSaleCategory &&
                lowerCase(item.food).includes(lowerCase(watchSearchKey))
            )
          )
        );
      ...
      case "special-dishes":
        dispatch(
          setDisplaySaleCategory(
            allProducts.filter(
              (item) =>
                item.sub_category === "Special Dishes" &&
                lowerCase(item.food).includes(lowerCase(watchSearchKey))
            )
          )
        );
    ...
    <div>
      ...
      <div className="parent-container">
        <div className="sales-payment-ch">
          {(isEmpty(clickCounts) ||
            sumSalesSubTotal(allProducts, clickCounts) === 0) && <Empty />}
          {!isEmpty(clickCounts) && (
              ...
              <tbody>
                    ...
                    <>
                      {clickCounts[item] > 0 && (
                        <tr key={i}>
                          ...
                          <td className="td-sales">
                            {(
                              allProducts.find((prod) => prod.id === item)
                                .minimum_price * clickCounts[item]
                            ).toLocaleString()}
                          </td>
                          <td className="td-sales">
                            <Radio.Group
                ...
                {sumSalesSubTotal(allProducts, clickCounts) > 0 && (
                  <>
                    ...
                    <td className="td-sales">{`${sumSalesSubTotal(
                      allProducts,
                      clickCounts
                    ).toLocaleString()}/=`}</td>
                    <td className="td-sales">
                      <div
                        className="btn-add-sales text-center"
                ...
                return (
                  ...
                  >
                    ...
                    {clickCounts[item.id] > 0 && (
                      <h6 className="text-center">{clickCounts[item.id]}</h6>
                    )}
                  </div>
                );
              })}
